...
public abstract class BaseRequest<R extends BaseRequest> {
    ...
    public BaseRequest(String url) {
        ...
        OkHttpGo go = OkHttpGo.getInstance();
    ...
    protected Call generateCall(Request request) {
        ...
        if (readTimeOut <= 0 && writeTimeOut <= 0 && connectTimeout <= 0 && sslParams == null && userCookies.size() == 0) {
            return OkHttpGo.getInstance().getOkHttpClient().newCall(request);
        } else {
            OkHttpClient.Builder newClientBuilder = OkHttpGo.getInstance().getOkHttpClient().newBuilder();
            ...
            if (userCookies.size() > 0) OkHttpGo.getInstance().getCookieJar().addCookies(userCookies);
