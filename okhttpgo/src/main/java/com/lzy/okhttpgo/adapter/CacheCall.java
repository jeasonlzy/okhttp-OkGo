...
import com.lzy.okhttpgo.cache.CacheManager;
...
public class CacheCall<T> implements Call<T> {
        ...
        if (cacheMode == CacheMode.IF_NONE_CACHE_REQUEST) {
            ...
            if (cacheEntity != null && !cacheEntity.isExpire()) {
                ...
                if (data == null || headers == null) {
                    //由于没有序列化等原因,可能导致数据为空
                    sendFailResultCallback(true, rawCall, null, OkHttpException.INSTANCE("没有获取到缓存,或者缓存已经过期!"), callback);
                } else {
            ...
            public void onResponse(okhttp3.Call call, okhttp3.Response response) throws IOException {
                ...
                if (responseCode == 304 && cacheMode == CacheMode.DEFAULT) {
                    if (finalCacheEntity == null) {
                        sendFailResultCallback(true, call, response, OkHttpException.INSTANCE("服务器响应码304，但是客户端没有缓存！"), finalCallback);
                    } else {
                        T data = finalCacheEntity.getData();
                        HttpHeaders headers = finalCacheEntity.getResponseHeaders();
                        if (data == null || headers == null) {
                            //由于没有序列化等原因,可能导致数据为空
                            sendFailResultCallback(true, call, response, OkHttpException.INSTANCE("没有获取到缓存,或者缓存已经过期!"), finalCallback);
                        } else {
                ...
                if (responseCode == 404 || responseCode >= 500) {
                    sendFailResultCallback(false, call, response, OkHttpException.INSTANCE("服务器数据异常!"), finalCallback);
                    return;
                }
    ...
    private void sendFailResultCallback(final boolean isFromCache, final okhttp3.Call call, final okhttp3.Response response, final Exception e, final AbsCallback<T> callback) {
        ...
        OkHttpGo.getInstance().getDelivery().post(new Runnable() {
            @Override
            public void run() {
                if (isFromCache) {
