...
public class DownloadThreadPool {
    ...
    private EndThreadPoolExecutor executor;                 //线程池执行器

    public EndThreadPoolExecutor getExecutor() {
        if (executor == null) {
            synchronized (DownloadThreadPool.class) {
                if (executor == null) {
                    executor = new EndThreadPoolExecutor(corePoolSize, MAX_IMUM_POOL_SIZE, KEEP_ALIVE_TIME, UNIT, //
